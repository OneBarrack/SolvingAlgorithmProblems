...
void DFS(vector<vector<bool>> visit, vector<string> board, map<char, pair<int, int>> tileList, char targetTile, int x, int y, vector<char> result, string& answer)
{
    if ( !answer.empty() || board[x][y] == '*' )
        return;

    visit[x][y] = true;
    if ( x == tileList[targetTile].first && y == tileList[targetTile].second )
        board[x][y] = '.';
    ...
    if ( tile == targetTile )
    {
        ...
        for ( auto iter = nextTileList.begin(); iter != nextTileList.end(); ++iter )
        {
            ...
            DFS(visit, board, nextTileList, nextTargetTile, nextTargetCoods.first, nextTargetCoods.second, result, answer);
        }
    }
    else if ( tile == '.' )
    {
        ...
        if ( y > 0 && !visit[x][y - 1] )
        {
            DFS(visit, board, tileList, targetTile, x, y - 1, result, answer);
        }

        // top
        if ( x > 0 && !visit[x - 1][y] )
        {
            DFS(visit, board, tileList, targetTile, x - 1, y, result, answer);
        }
...
string solution(int m, int n, vector<string> board)
{
    ...
    for ( auto iter = tileList.begin(); iter != tileList.end(); ++iter )
    {
        ...
        DFS(visit, board, tileList, targetTile, targetCoods.first, targetCoods.second, vector<char>(), answer);
    }
